...
- Cluster watch + WebSocket fan-out: `internal/watcher/`.
...
go test ./...         # must pass (tests live in internal/kubeconfig, etc.)
gofmt -l .            # must print nothing (format before finishing)
```

If available, also run `staticcheck ./...` and `govulncheck ./...`
(not currently installed in the dev env).
...
- Authorization is enforced per handler in `internal/server/server.go`:
  `RequireAuth` -> access check via `GetClustersForUser` -> `canUserModifyCluster`
  -> per-field `fieldEditable`. Any new mutating endpoint MUST replicate this
